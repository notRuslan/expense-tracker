...
- `feature/main-page` — главный экран (список транзакций, сводка)
...
PR — основная единица ревью. Один PR — одна логически связанная фича/фикс.

**Перед созданием PR:**

1. Свериться с актуальным `main`: `git fetch origin main`.
2. Прогнать `npm run build` и `npm run lint` затронутого workspace — CI должен быть зелёный с первой попытки.
3. Просмотреть `git diff origin/main...HEAD --stat` и пройтись глазами по изменениям — убедиться, что нет случайных файлов, секретов в `.env`, `console.log`, закомментированного кода.
4. Запушить ветку: `git push -u origin feature/<slug>`.

**Title:** в формате Conventional Commits (`<type>(<scope>): <описание>`), та же грамматика, что и в коммитах. Не более 72 символов. Пример: `feat(dashboard): add main page with paginated transactions`.

**Body** должен содержать секции (используем `gh pr create --body` через HEREDOC):

```markdown
## Что сделано
- 1-3 буллета по сути изменений (фокус на «зачем», а не «что»).

## Изменения по слоям
- **Backend:** перечень новых/изменённых эндпоинтов с методами и путями.
- **Frontend:** новые страницы/виджеты/фичи в терминах FSD.
- **Shared / Prisma / Infra:** что затронуто.

## Как проверить
- Шаги для ручной проверки (миграции, dev-серверы, какие сценарии прокликать).

## Чек-лист
- [ ] `npm run build` проходит
- [ ] Затронутые тесты зелёные
- [ ] Документация / CLAUDE.md обновлены при необходимости
- [ ] Известные ограничения / технический долг описаны ниже

## Технический долг (опционально)
- Что осознанно не сделано в рамках PR и почему.
```
...
- Не открывать PR с `WIP` в title — для черновика использовать `gh pr create --draft`.
- Не мержить PR в `main` без зелёного CI и одного аппрува.
- После мёржа удалять ветку (`git push origin --delete feature/<slug>`); локально — `git branch -d`.
- Связанные issue упоминать через `Refs: #<id>` или `Closes #<id>` в теле PR.
- Скриншоты/GIF UI-изменений — приветствуются, кладутся в раздел «Что сделано».
